 ...
 */
var checkout = {
    allowedPaymentProcessors: ['stripe', 'paypal', 'credit', 'coinbase', 'nowpayments'],
    paymentData: {},
    ...
    checkoutAmountValidation: function () {
        ...
        if ( (checkout.paymentData.type !== 'post-unlock' && (checkoutAmount.length > 0 && checkoutAmount >= 1 && checkoutAmount <= 500)) || checkout.paymentData.type === 'post-unlock') {
    ...
    checkoutBillingAgreementValidation: function () {
        ...
        let country = $('.country-select').find(':selected').val() !== '' && checkout.paymentData.country !== null && checkout.paymentData.country !== '';
        if (firstName && lastName && billingAddress && city && state && postcode && country) {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result !== null && typeof result.countries !== 'undefined' && result.countries.length > 0) {
                    $('.country-select').find('option').remove().end().append('<option value="">Select a country</option>');
